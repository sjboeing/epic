...
theta_array=np.ones((1,len_parcels))*theta_parcels
qv_array=np.ones((1,len_parcels))*qv_parcels
...
ds = xr.Dataset(
    {
        ...
        "theta": xr.DataArray(theta_array, dims=["time", "n_parcels"], coords=coords,
                              attrs={"units": "K", "long_name": "potential temperature"}),
        "qv": xr.DataArray(qv_array, dims=["time", "n_parcels"], coords=coords,
...
ds.to_netcdf("191125_rain_input.nc", unlimited_dims=["time"])
