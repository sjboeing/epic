...
def _bokeh_plot_hybrid(h5reader, h5reader_f, step_parcel, step_field, coloring, vmin, vmax, display=None, **kwargs):
    nsteps = h5reader.get_num_steps()
    extent = h5reader.get_box_extent()
    origin = h5reader.get_box_origin()
    ...
    nparcels= h5reader.get_num_parcels(step_parcel)
    ttime = h5reader.get_step_attribute(step=step_parcel, name='t')
    ...
    x, y, width, height, angle = h5reader.get_ellipses_for_bokeh(step_parcel)

    if coloring == 'aspect-ratio':
        print('hybrid aspect ratio not yet implemented')
        exit(0)
    elif coloring == 'vol-distr':
        print('hybrid vol-distr not yet implemented')
        exit(0)
    else:
        data = h5reader.get_dataset(step=step_parcel, name=coloring)
    ...
    field_data=h5reader_f.get_dataset(step=step_field, name=coloring)
    color_mapper_2= LinearColorMapper(palette=Viridis256_r, low=vmin, high=vmax)
    zoomed_image=ndimage.zoom(np.transpose(field_data), 10, order=1)
...
def bokeh_plot_hybrid(fname_parcel, fname_field, step_parcel, step_field, fmt='png',
                       coloring='aspect-ratio', display='full HD', **kwargs):

    h5reader = H5Reader()

    h5reader.open(fname_parcel)
    ...
    h5reader_f.open(fname_field)

    if h5reader_f.is_parcel_file:
        raise IOError('Should not be a parcel output file.')

    nsteps = h5reader_f.get_num_steps()

    if step_field > nsteps - 1:
        raise ValueError('Step number exceeds limit of ' + str(nsteps-1) + '.')
    ...
    graph = _bokeh_plot_hybrid(h5reader, h5reader_f, step_parcel, step_field, coloring, vmin, vmax, display, **kwargs)

    if fmt == 'png':
        export_png(graph, filename = 'parcels_'  + coloring + '_step_' + \
            str(step_parcel).zfill(len(str(nsteps))) + '.png')
    elif fmt == 'svg':
        export_svg(graph, filename = 'parcels_'  + coloring + '_step_' + \
            str(step_parcel).zfill(len(str(nsteps))) + '.svg')
    else:
        raise IOError("Bokeh plot does not support '" + fmt + "' format.")
